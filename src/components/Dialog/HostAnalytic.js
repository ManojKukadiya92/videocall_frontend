...
const HostAnalytic = (props) => {
  ...
  useEffect(() => {
    ...
    imagePath.push(
      hostData.image1 === "null" ? hostData.image : hostData.image1
    );
    imagePath.push(
      hostData.image2 === "null" ? hostData.image : hostData.image2
    );
    setVideo(hostData.video); // eslint-disable-next-line
  }, [hostData]);
  ...
  const poster = imagePath.map((poster, index) => {
    return (
      <CarouselItem
